...
export function findProductRanges(lines) {
  const ranges = [];
...
export function serializeBlock(block, indent) {
...
export function parseProduct(lines, range) {
...
export function parseArticleFromLines(title, contentLines, slugOverride) {
  ...
  const description = firstParagraph(blocks);
  const searchText = blocksToSearchText(title, blocks);

  return { slug, title, description, blocks, searchText };
}

export function parseMainExport(inputPath) {
  const raw = fs.readFileSync(inputPath, "utf8").replace(/\r/g, "");
  const lines = raw.split("\n");
  const ranges = findProductRanges(lines);
  return ranges.map((r) => parseProduct(lines, r));
}

export function writeGeneratedData(products, outputPath, headerComment) {
  const linesOut = [];
  linesOut.push(headerComment);
...
export function insertArticle(product, article, afterSlug) {
  if (product.articles.some((a) => a.slug === article.slug || a.title === article.title)) {
    return false;
  }
  ...
  const idx = product.articles.findIndex((a) => a.slug === afterSlug);
  if (idx === -1) {
    product.articles.push(article);
    return true;
  }

  product.articles.splice(idx + 1, 0, article);
  return true;
}
