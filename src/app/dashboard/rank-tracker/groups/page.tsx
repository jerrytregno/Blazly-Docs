    ...
    <PageDataGuard>
      <div className="grid gap-4 sm:grid-cols-3">
        {(rankings?.keywordGroups ?? []).length === 0 ? (
          <Card className="sm:col-span-3"><CardContent className="p-6 text-sm text-slate-500">No keyword groups yet.</CardContent></Card>
        ) : (
          rankings?.keywordGroups.map((group) => (
            <Card key={group.name}>
              ...
              <CardContent>
                ...
                <ul className="mt-4 space-y-1">{group.keywords.map((k) => <li key={k} className="text-xs text-slate-400">{k}</li>)}</ul>
              </CardContent>
            </Card>
          ))
        )}
