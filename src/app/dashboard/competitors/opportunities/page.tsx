        ...
        <Card>
          ...
          <CardContent className="space-y-5">
            {(rankings?.opportunityGaps ?? []).length === 0 ? (
              <p className="text-sm text-[#b8a9d9]">No gap analysis data yet.</p>
            ) : (
              rankings?.opportunityGaps.map((gap) => (
                <div key={gap.label}>
                  <div className="mb-1 flex justify-between text-sm text-[#d4c4f5]"><span className="font-medium">{gap.label}</span><span>{gap.score}/100</span></div>
                  <ProgressBar value={gap.score} />
                  <p className="mt-1 text-xs text-[#9b8ab8]">{gap.insight}</p>
                </div>
              ))
            )}
          </CardContent>
