...
function syncStatusBadge(status?: string) {
  switch (status) {
    case "synced":
      return <Badge className="bg-emerald-500/20 text-emerald-200">Synced</Badge>;
    case "issues":
      return <Badge className="bg-amber-500/20 text-amber-200">Needs attention</Badge>;
    default:
      return <Badge className="bg-violet-500/20 text-violet-200">Pending</Badge>;
  }
}
      ...
      <div className="space-y-6">
        <Card className="border-fuchsia-500/20">
          ...
          <CardContent className="space-y-4">
            <div className="flex flex-wrap items-center gap-3 text-sm">
              <span className="text-[#9b8ab8]">Listing sync:</span>
              {syncStatusBadge(dashboard?.listingSyncStatus)}
              {audit && (
                <span className="text-[#9b8ab8]">
                  NAP score: <span className="font-medium text-white">{audit.score}%</span>
                </span>
              )}
        ...
        {audit && audit.score < 80 && (
          <Card className="border-amber-500/20 bg-amber-500/5">
            <CardContent className="p-4 text-sm text-[#d4c4f5]">
              <p className="font-medium text-white">Recommended fixes</p>
              <ul className="mt-2 list-inside list-disc space-y-1 text-[#b8a9d9]">
                {audit.fields
                  .filter((f) => !f.consistent)
                  .map((f) => (
                    <li key={f.field}>
                      Align <span className="text-white">{f.field}</span> — GBP shows &quot;{f.gbpValue}&quot;, your profile has &quot;{f.webValue}&quot;
                    </li>
                  ))}
              </ul>
              <Link
                href="/dashboard/gbp/business-details"
                className={cn("mt-3 inline-block text-violet-300 hover:text-violet-200")}
              >
